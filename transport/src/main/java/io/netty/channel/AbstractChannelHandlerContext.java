...
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
     ...
     * 初始状态，handlerAdded和handlerRemoved都没有被调用
     */
    ...
    // 表示事件执行器ChannelHandler的执行标记，用来判断是否跳过执行器ChannelHandler的某些事件处理方法
    private final int executionMask;

    // Will be set to null if no child executor should be used, otherwise it will be set to the child executor.
    // 上下文的执行器，如果这个值是null，那么上下文的执行器用的就是所属通道Channel的事件轮询器
    ...
    // handlerState是通过HANDLER_STATE_UPDATER, 采用CAS的方式原子化更新的状态
    ...
    AbstractChannelHandlerContext(DefaultChannelPipeline pipeline, EventExecutor executor,
        ...
        // executor == null，说明当前上下文用的是通道Channel的channel().eventLoop()，这个肯定是有序的
    ...
    public ChannelHandlerContext fireChannelRegistered() {
        // 通过findContextInbound方法找到下一个入站处理器上下文，通过invokeChannelRegistered方法，调用下一个入站处理器的对应事件处理方法
        invokeChannelRegistered(findContextInbound(MASK_CHANNEL_REGISTERED));
        return this;
    }

    /**
     * 保证在上下文next的事件执行器线程中调用对应方法
     */
    static void invokeChannelRegistered(final AbstractChannelHandlerContext next) {
        ...
        if (executor.inEventLoop()) {
            ...
            // 如果当前线程不是next事件执行器线程，则通过事件执行器EventExecutor的execute方法，保证在执行器线程调用
    ...
    /**
     * 调用该上下文拥有的事件处理器ChannelHandler的对应方法
     */
    private void invokeChannelRegistered() {
        ...
        if (invokeHandler()) {
            ...
            // 如果没有添加完成，即状态不是ADD_COMPLETE，就继续调用fire的方法，让管道下一个处理器处理
    ...
    /**
     * 在处理I/O操作期间如果遇到异常，会被调用。这个方法会通知下一个处理器（handler）关于这个异常的信息
     *
     * 异常处理是通过一个专门的ExceptionHandler来进行的。当一个异常抛出时，Netty会查找ChannelPipeline中的下一个ExceptionHandler来处理这个异常
     * 如果在ChannelPipeline中没有找到合适的ExceptionHandler来处理这个异常，那么这个异常就会被抛出，并且会导致Channel被直接关闭
     */
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("An exception {}was thrown by a user handler's exceptionCaught() " +
                        "method while handling the following exception:", ThrowableUtil.stackTraceToString(error), cause);
    ...
    private void invokeBind(SocketAddress localAddress, ChannelPromise promise) {
        ...
        if (invokeHandler()) {
            ...
            // 如果没有添加完成，即状态不是 ADD_COMPLETE，就继续调用 fire 的方法，让管道下一个处理器处理
    ...
    private void invokeConnect(SocketAddress remoteAddress, SocketAddress localAddress, ChannelPromise promise) {
        if (invokeHandler()) {
            try {
                // 这里会调用DefaultChannelPipeline的HeadContext的connect方法
    ...
    public ChannelHandlerContext flush() {
        ...
        if (executor.inEventLoop()) {
            ...
            // 因为刷新操作没有参数，不用每次都创建新的Runnable实例，直接复用
            Tasks tasks = next.invokeTasks;
            if (tasks == null) {
                next.invokeTasks = tasks = new Tasks(next);
            }
    ...
    private void invokeFlush() {
        if (invokeHandler()) {
            ...
            // 当前上下文对应的处理器不处理，继续在管道中寻找下一个事件处理器处理
            flush();
        }
    }
    ...
    private AbstractChannelHandlerContext findContextInbound(int mask) {
        ...
        do {
            ctx = ctx.next;
            // 通过MASK_ONLY_INBOUND表示查找的是入站事件，mask代表处理的方法是否需要被跳过
    ...
    private AbstractChannelHandlerContext findContextOutbound(int mask) {
        ...
        do {
            ctx = ctx.prev;
            // 通过MASK_ONLY_OUTBOUND表示查找的是出站事件，mask代表处理的方法是否需要被跳过
    ...
    private static boolean skipContext(AbstractChannelHandlerContext ctx, EventExecutor currentExecutor, int mask, int onlyMask) {
        ...
        // 这个方法返回true，表示跳过这个ctx，继续从管道中查找下一个，因为使用的是||或逻辑符，两个条件只要有一个为true，就返回true
        // (ctx.executionMask & (onlyMask | mask)) == 0表示这个ctx属于入站事件还是出站事件
        // (ctx.executor() == currentExecutor && (ctx.executionMask & mask) == 0)
        // 只有当EventExecutor相同的时候，才会考虑是否跳过ctx，因为我们要保证事件处理的顺序
    ...
    final boolean setAddComplete() {
        for (;;) {
            ...
            // 确保当handlerState已经是REMOVE_COMPLETE时，我们永远不会更新
            // oldState通常是ADD_PENDING，但当使用不公开排序保证的EventExecutor时，也可能是REMOVE_COMPLETE。
    ...
    final void setAddPending() { // 等待添加
        boolean updated = HANDLER_STATE_UPDATER.compareAndSet(this, INIT, ADD_PENDING);
        // 这应该总是为真，因为它必须在setAddComplete()或setRemoved()之前调用。
    ...
    final void callHandlerAdded() throws Exception {
        ...
        // 我们必须在调用handlerAdded之前调用setAddComplete，将状态改成REMOVE_COMPLETE，
        // 否则这个上下文对应的事件处理器将不会处理任何事件，因为状态不允许
        if (setAddComplete()) {
            // 如果设置成功就调用 handler().handlerAdded(this) 方法，通知事件处理器已经被添加到管道上了
    ...
    final void callHandlerRemoved() throws Exception {
        try {
            ...
            // 只有handlerState状态变成ADD_COMPLETE时，才会调用handler().handlerRemoved(this)
            // 也就是说只有之前调用过handlerAdded(…)方法，之后才会调用handlerRemoved(…)方法
     ...
     *
     * 尽最大努力检测ChannelHandler.handlerAdded(ChannelHandlerContext)是否被调用，如果没有被调用则返回false，如果调用或无法检测返回true
     *
     * 如果这个方法返回false，我们将不调用ChannelHandler，而只是转发事件，调用管道中下一个ChannelHandler处理。
     *
     * 因为可能管道DefaultChannelPipeline已经将这个ChannelHandler放在链接列表中，但没有调用ChannelHandler.handlerAdded(ChannelHandlerContext)方法，
     * 有可能用户在ChannelHandler.handlerAdded中做了一些初始化操作，当它没有被调用时，不能将IO事件交个这个ChannelHandler处理
     */
